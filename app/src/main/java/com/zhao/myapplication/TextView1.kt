...
class TextView1 : View {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        var sum = 0
        while (sum < 360) {
            canvas.rotate(20+rotate.toFloat(), width / 2F, height / 2F)
            if (sum == 0){
                settingPaint(Paint.Style.FILL, R.color.purple_700, 5F, Paint.Cap.SQUARE)
                canvas.drawText("开始", width / 2F, height - 100F, paint)
            }
            settingPaint(Paint.Style.STROKE, R.color.purple_200, 15F, Paint.Cap.ROUND)
            canvas.drawArc(15F, 15F, width - 15F, height - 15F, 90F, 30F, false, paint)
            sum += 30
        }
    }

    fun start() {
        val valueAnimator: ValueAnimator = ValueAnimator.ofInt(0, 10)
